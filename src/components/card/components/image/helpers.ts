...
export function loadImage(source: string, imgRef: HTMLImageElement | null, promiseResolveTimeout?: number): Resource<string> | undefined {
  if (!imgRef) return;
  ...
  resource = createResource<string>(
    () =>
      ...
      new Promise((resolve, reject) => {
        ...
        promiseResolveTimeout ? setTimeout(() => imageLoad(), promiseResolveTimeout) : imageLoad();
      })
  );
